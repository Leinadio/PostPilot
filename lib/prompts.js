...
function buildPrompt(type, postContent, polarity, examples, wordCount) {
  ...
  systemParts.push("ATTAQUE DIRECTE. Ton commentaire entre directement dans le sujet. La première phrase ne qualifie pas le post, ne le résume pas, ne l'évalue pas. Tu ne commentes pas le post, tu réagis au contenu. Aucune phrase du commentaire ne reformule ce que le post dit avec d'autres mots. Si le commentaire commence par une phrase qui parle du post plutôt que du sujet, il sera rejeté. Si une phrase du commentaire paraphrase ou résume un point du post au lieu d'apporter du contenu nouveau, le commentaire sera rejeté. Ne fais jamais référence à l'auteur du post, à ce qu'il dit, mentionne, soulève ou propose. Le commentaire existe comme une prise de position indépendante sur le sujet. Si le commentaire contient une référence à l'auteur ou au post lui-même, il sera rejeté");

  systemParts.push("VARIÉTÉ SYNTAXIQUE. Chaque phrase du commentaire a une construction grammaticale distincte des autres. Si deux phrases démarrent de la même façon, suivent le même patron structurel, ou utilisent le même mécanisme de mise en relief, le commentaire sera rejeté. Aucune tournure ne se répète à travers le commentaire, ni en ouverture de phrase, ni dans l'enchaînement sujet-verbe, ni dans le rythme global. Un commentaire dont on peut prédire la forme de la phrase suivante sera rejeté");
  ...
  systemParts.push("ZÉRO ANALOGIE. Le commentaire reste dans le champ thématique du post. Pas de comparaison avec un autre domaine, pas de métaphore empruntée à un univers extérieur au sujet. Si le commentaire introduit un domaine étranger au post pour illustrer son propos, il sera rejeté");
  ...
  systemParts.push("ZÉRO CONNECTEUR LOGIQUE. Les phrases s'enchaînent sans mot de liaison et sans mot-étiquette entre elles. Chaque phrase commence directement par son contenu. Aucun mot n'annonce la relation logique avec ce qui précède, que ce soit une transition, un résumé, ou une étiquette de rôle. Si une phrase commence par un mot ou une expression dont la fonction est de signaler comment elle se rattache à la précédente, le commentaire sera rejeté");

  systemParts.push("CONCRET AVANT ABSTRAIT. Chaque mot du commentaire désigne quelque chose de tangible ou d'observable. Si un mot pourrait être remplacé par n'importe quel autre mot vague sans changer le sens de la phrase, ce mot est trop abstrait. Un commentaire rempli de termes génériques qui ne désignent rien de précis sera rejeté");
  ...
  systemParts.push("ARC IMPRÉVISIBLE. Le commentaire ne suit pas un schéma logique linéaire. La deuxième phrase ne justifie pas la première et la dernière phrase ne conclut pas le propos. Les idées se juxtaposent sans hiérarchie apparente. Si on peut résumer la structure du commentaire comme \"affirmation puis justification puis conclusion\", il sera rejeté");
