...
var buildResizePrompt = function(direction, comment, postContent, ton) {
  if (direction === 'shorter') {
    var system = 'Zéro hashtag. Zéro émoji. Zéro tiret cadratin (—). Zéro tiret demi-cadratin (–). Zéro deux-points (:). Un seul de ces caractères et le commentaire sera rejeté.\n\nCe commentaire LinkedIn contient plusieurs phrases. Supprime la phrase la moins importante. Garde les autres phrases telles quelles. Tu peux ajuster légèrement une transition pour la fluidité, mais si tu reformules entièrement les phrases restantes, le commentaire sera rejeté. Même ton, même registre. Retour à la ligne entre chaque phrase.\n\nSi le commentaire n\'a qu\'une seule phrase, reformule-la plus courte sans changer le sens.';
    var user = "Commentaire à raccourcir\n\n" + comment + "\n\nRAPPEL FORMATAGE. Aucun deux-points, tiret cadratin, tiret demi-cadratin, emoji, hashtag. Un seul et le commentaire sera rejeté.\n\nPlusieurs phrases. supprime-en une, garde les autres quasi identiques. Une seule phrase. reformule-la plus courte. Dans les deux cas, si tu réécris tout, le commentaire sera rejeté.\n\nRetourne UNIQUEMENT le commentaire raccourci.";
    return { system: system, user: user };
  }

  var systemParts = [
    'Zéro hashtag. Zéro émoji. Zéro tiret cadratin (—). Zéro tiret demi-cadratin (–). Zéro deux-points (:). Un seul de ces caractères et le commentaire sera rejeté.\n\nTu n\'as PAS de passé, pas de mémoire, pas de relations, pas d\'entourage. Toute phrase qui prétend que tu as un vécu personnel est un mensonge. Ne fabrique jamais de faux souvenirs ou de fausses anecdotes.',
    'Ajoute une phrase à ce commentaire LinkedIn. La phrase ajoutée prolonge naturellement le propos avec quelque chose de concret, pas du remplissage. La phrase ajoutée est dans la même langue que le commentaire. Si le commentaire a déjà 4 phrases ou plus, retourne-le tel quel sans rien ajouter. Même ton, même registre. Retour à la ligne entre chaque phrase.'
  ];
  ...
  var tonRappel = '';
  if (ton === 'familier') {
    tonRappel = "\n2. TON. Tutoiement, décontracté, vocabulaire simple. Si le commentaire vouvoie ou sonne formel, il sera rejeté.";
  } else if (ton === 'pro') {
    tonRappel = "\n2. TON. Vouvoiement, professionnel mais naturel. Si le commentaire tutoie, il sera rejeté.";
  } else if (ton === 'neutre') {
    tonRappel = "\n2. TON. Impersonnel, pas d'adresse directe, pas de \"je\" dominant. Si le commentaire s'adresse directement à l'auteur, il sera rejeté.";
  }

  var user = "Post original\n\n" + postContent + "\n\nCommentaire (ajoute une phrase)\n\n" + comment + "\n\nRAPPELS\n1. FORMATAGE. Aucun deux-points, tiret cadratin, tiret demi-cadratin, emoji, hashtag. Un seul et le commentaire sera rejeté." + tonRappel + "\n\nRetourne UNIQUEMENT le commentaire développé.";
